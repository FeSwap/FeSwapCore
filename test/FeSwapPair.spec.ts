...
describe('FeSwapPair', () => {
  ...
  it('Swap: Gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq("57730")      // 58257 58308 58330 71350 71365 72482 72460
  })
  ...
  it('Burn: Gas', async () => {
    ...
    const receipt = await tx.wait()
    expect(receipt.gasUsed).to.eq("88242")     // 88808 89594 89696 89673 89695 132725 132681  133481 134278 148203 149984 149961 148288  //different liquity ,gas could be different
  })
  ...
  it('Swap Arbitrage Gas：no  feeTo and pairOwner fee', async () => {
    ...
    expect(receipt.gasUsed).to.eq("103642")      // 103677 104306 104313 105957 105964 105979 106008 118968 119033 119833 121500 121449  121426 109422 123994 131668 157206  //241214
    ...
    expect(feeToAAB).to.eq(0)           
    expect(feeCreateAAB).to.eq(0)   
    expect(receipt.gasUsed).to.eq("103642")      // 103677 104306 105979 118968 119833 121500 121449  121426 109422 123994 131668 157206  //241214
  ...
  it('Swap Arbitrage Gas：no  feeTo, but pairOwner fee on', async () => {
      ...
      expect(feeCreateAAB).to.eq(0)  
      expect(receipt.gasUsed).to.eq("152986")      // 153021 153728 153735 155379 155386 155401 155430 168390 168455 169255 157095 170706 170655 170632 173200 180874 157206  //241214
      ...
      expect(feeToAAB).to.not.eq(0)           
      expect(feeCreateAAB).to.eq(0)  
      expect(receipt.gasUsed).to.eq("137986")      // 138728 138735 140379 140401 140430 153390 153455 168455
...
//    await router.connect(pairOwner).ManageFeswaPair(tokenIDMatch, constants.AddressZero, rateTriggerArbitrage) 
      ...
      expect(receipt.gasUsed).to.eq("154612")      //155378 155385 157029 157051 157080 170040 170105  170905 158738 159535 172332 160328  172281 172258 174826, 162822 157206  //241214
      ...
      expect(receipt.gasUsed).to.eq("139612")      //140378 140385 142029 142051 142080 155040 155105 170105
...
//    await factory.setFeeTo(constants.AddressZero) 
      ...
      expect(receipt.gasUsed).to.eq("178374")      //179152 179159 180803 180825 180854 193814 193879 194679 182519 183309 194608 196094 184039 196020 206262 157206  //241214
      ...
      expect(receipt.gasUsed).to.eq("148374")         // 149152 149159 150803 150825 150854 163814 163879 Less than 1st tx
    ...
    it('Swap Arbitrage Gas comparsion', async () => {
      ...
      await mineBlock(provider, blockTimestamp + 5)
        ...
        let tx = await router.swapExactTokensForTokens( swapAmount, 0,  [tokenB.address, tokenA.address],
                                                wallet.address, constants.MaxUint256,  overrides )
        let receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq("90569")     // 93366 92377 92428 92450 135470 135658 136305 94279 136272 94317 136321 136343 90889
        ...
        tx = await router.swapExactTokensForTokens( swapAmount, 0,  [tokenB.address, tokenA.address],
                                                wallet.address, constants.MaxUint256,  overrides )
        receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq("90569")     //92377 105470  93310  105470
      }
      {
        const swapAmount = expandTo18Decimals(10)
        await mineBlock(provider, blockTimestamp + 20)  
        ...
        let tx = await router.swapExactTokensForTokens( swapAmount, 0,  [tokenB.address, tokenA.address],
                                              wallet.address, constants.MaxUint256,  overrides )
                                              let receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq("152356")   // 155728 155620 155693 182561 170401 173899 173866 161862 173866 185915 173915 174757  //241214

        await mineBlock(provider, blockTimestamp + 25)  
        tx = await router.swapExactTokensForTokens( swapAmount, 0,  [tokenB.address, tokenA.address],
                                              wallet.address, constants.MaxUint256,  overrides )
        receipt = await tx.wait()
        expect(receipt.gasUsed).to.eq("152356")   // 155671 158241 182561 
      }
    })
})
