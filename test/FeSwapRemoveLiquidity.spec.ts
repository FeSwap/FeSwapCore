...
describe('FeSwapRemoveLiquidity', () => {
    ...
    const [wallet, feeTo, pairCreator, other] = provider.getWallets()
    const loadFixture = createFixtureLoader(provider, [wallet, feeTo, pairCreator, other])
...
//    [10].forEach( (ratio) => {
    ...
    it("removeLiquidity GAS Test" , async () => {
        ...
        const tx = await router.removeLiquidity(
          ...
          expect(receipt.gasUsed).to.eq(290718)    //246210, 246129 258878  Uniswap: 253427
      }).retries(3) 
      ...
      it(`removeLiquidity ratio AAB: 100-0：Gas test`, async () => {
        ...
        const tx = await router.removeLiquidity(
          ...
          const receipt = await tx.wait()
          expect(receipt.gasUsed).to.eq(161663)    // 139320, 139258,  246129,  Uniswap: ???
      }).retries(3) 
...
//      [10].forEach( (ratio) => {              
      ...
      it("RemoveLiquidityETH GAS Test" , async () => {
        ...
        const tx = await router.removeLiquidityETH(
          ...
          const receipt = await tx.wait()
          expect(receipt.gasUsed).to.eq(316181)        // 271629, 271701, 284451 : Uniswap: 194881
      }).retries(3) 
      ...
      it(`removeLiquidityETH TTE ratio: 100-0: GAS Usage `, async () => {
        ...
        const tx = await router.removeLiquidityETH(
          ...
          const receipt = await tx.wait()
          expect(receipt.gasUsed).to.eq(186485)        // 164077,   164124,  284451 : Uniswap: 194881
      }).retries(3) 
